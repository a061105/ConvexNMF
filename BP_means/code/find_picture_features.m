function [Z,A,pics] = find_picture_features(R_path, directory, pic_format, varargin)
...
if(length(options.pca_file) == 0)
	...
	for(n = 1:N)
		...
		%pic_file_name = [directory '/Picture_' num2str(n) '.jpg']
		%img = imread(pic_file_name);
		%img=rgb2gray(img); img=img(1:3:end,1:3:end); img(img<=40)=0;
		%size(img)
		%this_pic=img;
		%pic_size = size(this_pic); %14 * 14
