...
% tic;
% [That, Ahat, status] = Integerfac_findvert_cpp(D, 4, [0 1], opt0);
% toc;
% %%% *** [6] table *** %%%
%  disp('table');
%  load('../../tabletop.mat')
...
%  opt0 = opt_Integerfac_findvert('nonnegative', false, 'affine', false);
%  tic;
%  [That, Ahat, status] = Integerfac_findvert_cpp(D, 5, [0 1], opt0);
%  toc;
%  %%% *** [7] big_feature *** %%%
%  % disp('k=25');
%  % load('../../big_feature.mat')
%  % D=pics;
%  % opt0 = opt_Integerfac_findvert('nonnegative', false, 'affine', false);
%  % tic;
%  % [That, Ahat, status] = Integerfac_findvert_cpp(D, 15, [0 1], opt0);
%  % toc;
%   %%% *** [7] big_feature *** %%%
 disp('medical');
 load('../../R_medical.mat')
 ...
 opt0 = opt_Integerfac_findvert('nonnegative', false, 'affine', false);
 tic;
 [That, Ahat, status] = Integerfac_findvert_cpp(D,45, [0 1], opt0);
 toc; 
%%% *** End of file *** %%%%%%%%%%%%%%%%%%
